...
public class Controller implements Initializable {
    ...
    private void startGameButtonClicked(ActionEvent event) { // 숫자인 입력값일때
        ...
        try {
            ...
            if(tempPlayerNum < 2 || tempPlayerNum > 4 || (tempPieceNum<2 || tempPieceNum > 5)){
                ...
                yutnoriModel.playingPlayer.get(0).playerPieces.get(0).onBoard = true; // 첫번째 플레이어의 첫번쩌 piece가 board 위에 있게 설정
                Piece p = yutnoriModel.playingPlayer.get(0).playerPieces.get(0);
                yutnoriModel.board.addPiece(1, yutnoriModel.playingPlayer.get(0).playerPieces.get(0));
            }
    ...
    private void listViewSetOnMouseClicked(MouseEvent event){
        ...
        int pieceStart = 1;
        // squaresView.get(1 + 3);
        for(int i = 1; i <= (yutnoriModel.board.squares)[1].onSquarePieces.size(); i++){ // 원래 있던 square의 circle들을 안보이게
            squaresView[1][i].setVisible(false);
        }
        yutnoriModel.board.movePiece(1, 3);
        for(int i = 1; i <= (yutnoriModel.board.squares)[1 + 3].onSquarePieces.size(); i++){ // 도착 square의 circle들을 보이게
