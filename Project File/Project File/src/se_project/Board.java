...
public class Board {
    ...
    void addPiece(int position, Piece piece){   // 유저가 갖고 있는 Piece의 인스턴스를 여기다가도 reference
        squares[position].onSquarePieces.add(piece);
    }

    void movePiece(int position, int moveDistance) {
        /* 말을 움직이는 메소드 */
        int size = squares[position].onSquarePieces.size();
        Piece p;
        for(int i = 0; i < size; i++){                  // 원래 있던 square의 piece를 목표 square의 piece로 이동
            p = squares[position].onSquarePieces.get(i);
            squares[position + moveDistance].onSquarePieces.add(p);
        }
        squares[position].onSquarePieces.clear();
    }
    void removePiece() {
        /* 잡힌 말을 제거하는 메소드 */
    }
